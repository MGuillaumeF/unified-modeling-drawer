...
interface ConfigurationManagerParams {
  filename?: string;
  strict?: boolean;
...
export default class ConfigurationManager {
  ...
  private constructor(params?: ConfigurationManagerParams) {
    this._filepath = path.resolve(
      process.cwd(),
      params?.filename ? params?.filename : "configuration.json"
    );
    this._strict = params?.strict ?? false;
    this._onError = params?.onError;
    try {
      if (existsSync(this._filepath)) {
        const configurationFileContent = JSON.parse(
          readFileSync(this._filepath).toString()
        );
        if (ConfigurationManager.isValid(configurationFileContent)) {
          this._configuration = configurationFileContent;
        } else {
          this.manageError(
            new ConfigurationError("invalid configuration file content")
          );
        }
      } else if (params?.filename) {
        this.manageError(
          new ConfigurationError(
            `configuration file not found: ${this._filepath}`
          )
        );
      }
    } catch (e) {
      ...
      this.manageError(
        new ConfigurationError("invalid configuration file reading", option)
      );
    }
  }
  public static getInstance(
    params?: ConfigurationManagerParams
