...
import ConfigurationManager, {
  ConfigurationError
} from "../../src/ConfigurationManager";
import path from "path";
import { existsSync, writeFileSync, unlinkSync, readFileSync } from "fs";

const INVALID_JSON_CONFIG = path.resolve(
  process.cwd(),
  "invalid_configuration.json"
);
const VALID_JSON_CONFIG = path.resolve(
  process.cwd(),
  "invalid_configuration.json"
);
...
describe("ConfigurationManager Test Suite", function () {
        ...
        if (e instanceof ConfigurationError) {
          ...
          if (e?.cause instanceof ConfigurationError) {
            ...
            expect(function () {
              ...
              `configuration file not found: ${path.resolve(
                process.cwd(),
                "notExist"
              )}`
            );
  ...
  it("Call instance with existing bad json file in strict mode", function () {
    writeFileSync(
      INVALID_JSON_CONFIG,
      JSON.stringify({ content: "no language" }, null, 2)
    );
    expect(function () {
      try {
        ConfigurationManager.getInstance({
          filename: INVALID_JSON_CONFIG,
          strict: true
        });
      } catch (e) {
        expect(e instanceof ConfigurationError).to.equal(
